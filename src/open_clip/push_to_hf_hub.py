...
from .tokenizer import CLIP_TOKENIZER_TO_WRAP, HFTokenizer
...
def save_for_hf(
        ...
        model_config: Mapping[str, Any] | None,
        save_directory: str,
        safe_serialization: bool | str = "both",
...
def push_to_hf_hub(
        ...
        safe_serialization: bool | str = False,
):
    if not isinstance(tokenizer, HFTokenizer):
        # FIXME: this makes it awkward to push models with new tokenizers, come up with better solution.
        tokenizer = HFTokenizer(CLIP_TOKENIZER_TO_WRAP)
...
def push_pretrained_to_hf_hub(
    ...
    model, transform_val = create_model_from_pretrained(
        model_name,
